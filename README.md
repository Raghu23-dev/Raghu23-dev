...
| **[sentinel](https://github.com/Raghu23-dev/sentinel)** | Human-in-the-loop approval for agents. The timeout direction follows the risk — recoverable work proceeds unattended, irreversible work does not — and **eight distinct failure modes all produce a refusal rather than a permit**, because a permission check that cannot complete has not granted permission. | `Python` · `SQLite` · `Hypothesis` |
